...
#define STR_TOSS_ROPES "Lançar Corda"
#define STR_FAST_ROPE "Descer pela Corda"
#define STR_CUT_ROPES "Cortar Corda"
...
zlt_rope_helis = ["B_Heli_Transport_03_black_F","O_Heli_Transport_04_bench_F","B_Heli_Light_01_F","O_Heli_Light_02_unarmed_F","I_Heli_Transport_02_F","B_Heli_Transport_01_F","B_Heli_Transport_03_unarmed_F"];
...
player addAction["<t color='#00ff00'>"+STR_FAST_ROPE+"</t>", zlt_fnc_fastrope, [], 15, false, false, '','not zlt_mutexAction AND count ((vehicle player) getVariable ["zlt_ropes", []]) != 0 AND ((getPos (vehicle player) select 2) < MAX_ALTITUDE_ROPES_AVAIL) AND locked vehicle player == 0 AND player != driver vehicle player && !(player getVariable "restrained")'];
